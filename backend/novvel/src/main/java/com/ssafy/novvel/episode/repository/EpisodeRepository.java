...
public interface EpisodeRepository extends JpaRepository<Episode, Long> {
    ...
    @Query("SELECT ep.id FROM Episode ep WHERE ep.publishedDate = " +
            "(SELECT MAX(ep.publishedDate) FROM Episode ep WHERE ep.cover = :cover AND ep.publishedDate < :time)")
    Long findPrevEpisodeId(@Param("time") LocalDateTime time, @Param("cover") Cover cover);

    @Query("SELECT ep.id FROM Episode ep WHERE ep.publishedDate = " +
            "(SELECT MIN(ep.publishedDate) FROM Episode ep WHERE ep.cover = :cover AND ep.publishedDate > :time)")
    Long findNextEpisodeId(@Param("time") LocalDateTime time, @Param("cover") Cover cover);
