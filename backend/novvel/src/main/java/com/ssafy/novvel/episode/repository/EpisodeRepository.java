...
public interface EpisodeRepository extends JpaRepository<Episode, Long> {
    ...
    @Query("SELECT ep.id FROM Episode ep WHERE ep.id = (SELECT MAX(ep.id) FROM Episode ep WHERE ep.statusType = 'PUBLISHED'" +
            "AND ep.cover = :cover AND ep.id < :id)")
    Long findPrevEpisodeId(@Param("id") Long id, @Param("cover") Cover cover);

    @Query("SELECT ep.id FROM Episode ep WHERE ep.id = (SELECT MIN(ep.id) FROM Episode ep WHERE ep.statusType = 'PUBLISHED' " +
            "AND ep.cover = :cover AND ep.id > :id)")
